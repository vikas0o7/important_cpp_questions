...
int main()
{
	...
	while(m--){
		...
		g[y].pb(x);  /// to store reverse of graph for using later for second dfs;
	}
	dfs1(0);
	// while(!st.empty()){
	// 	cout<<st.top()<<endl; st.pop();        // debug:to check ordering of elements in stack
	// }
	int cnt=0;                //// to count the number of strongly conn. components;
	memset(vis,false,sizeof(vis));
	while(!st.empty()){
		int a=st.top(); st.pop();
		if(!vis[a]){ cnt++;
